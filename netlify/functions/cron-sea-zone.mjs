// 해구별 기상 예보 자동 수집 함수 (스케줄: 00:30, 01:30, 12:30, 13:30)
// 모든 해구 데이터를 수집하여 Netlify Blob에 저장
...
export default async function handler(request, context) {
    console.log('⏰ [Cron] 해구별 기상 전체 갱신 시작...');
    const now = Date.now();
    ...
    for (const code of SEA_ZONE_CODES) {
        try {
            ...
            console.warn(`[Cron] 해구 ${code} 조회 실패:`, e.message);
        }
    }

    const count = Object.keys(allData).length;
    console.log(`⏰ [Cron] 수집 완료: ${count}개 해구`);
